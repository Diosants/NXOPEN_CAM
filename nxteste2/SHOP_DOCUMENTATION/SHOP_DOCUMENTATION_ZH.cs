...
{
    public class SHOP_DOCUMENTATION_ZH
    {
        // ================================================================
        // >>> PERSONALIZACAO POR 客户 - EDITE AQUI <<<
        //
        ...
        private const string TITULO_DOCUMENTO = "工艺流程单";
        ...
        public static void Run(string[] args)
        {
            ...
            // ----------------------------------------------------------------
            // 1) Percorre as 道工序, agrupando tudo por FERRAMENTA
            // ----------------------------------------------------------------
            Dictionary<string, DadosFerramenta> ferramentas = new Dictionary<string, DadosFerramenta>();
            List<OperacaoOrdenada> 道工序Ordenadas = new List<OperacaoOrdenada>();
            ...
            foreach (CAMObject camObj in workPart.CAMSetup.CAMOperationCollection)
            {
                ...
                // real da colecao de 道工序 do CAM), independente de
                // ter conseguido achar a ferramenta ou nao.
                道工序Ordenadas.Add(new OperacaoOrdenada
                {
            ...
            html.AppendLine(string.Format("<div class='info'>{0} &nbsp;&nbsp;|&nbsp;&nbsp; 生成于 {1} &nbsp;&nbsp;|&nbsp;&nbsp; {2} 道工序 &nbsp;&nbsp;|&nbsp;&nbsp; {3} 把刀具</div>",
            ...
            html.AppendLine(LinhaInfoEditavel("编程员", programador));
            html.AppendLine(LinhaInfoEditavel("公司", ""));
            html.AppendLine(LinhaInfoEditavel("客户", ""));
            html.AppendLine(LinhaInfoEditavel("机床", ""));
            html.AppendLine(LinhaInfoEditavel("数控系统", ""));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<table class='tabela-info'>");
            html.AppendLine(LinhaInfoEditavel("工单号", "Nº"));
            html.AppendLine(LinhaInfoEditavel("日期", dataGeracao.Split(' ')[0]));
            html.AppendLine(LinhaInfoEditavel("交货期", ""));
            html.AppendLine(LinhaInfoEditavel("零件数量", "Nº"));
            html.AppendLine(LinhaInfoEditavel("操作员", "姓名："));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            html.AppendLine("<div class='coluna-esquerda-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>原材料尺寸</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("长度", "X", temBloco ? blocoL.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("宽度", "Y", temBloco ? blocoW.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("高度/厚度", "Z", temBloco ? blocoH.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>零件尺寸</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("长度", "X", temPeca ? pecaL.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("宽度", "Y", temPeca ? pecaW.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("高度/厚度", "Z", temPeca ? pecaH.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            if (temBloco || temPeca)
            {
                ...
                if (temBloco)
                {
                    ...
                    html.AppendLine("<div class='vista-titulo'>原材料 - 等轴测视图</div>");
                    html.AppendLine(GerarDiagramaBloco(blocoL, blocoW, blocoH, "materiaprima"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>长度: {0:F0}mm &nbsp;|&nbsp; 宽度: {1:F0}mm &nbsp;|&nbsp; 高度: {2:F0}mm</div>", blocoL, blocoW, blocoH));
                    html.AppendLine("</div>");
                }
                ...
                {
                    html.AppendLine("<div class='vista-setup vista-dim'>");
                    html.AppendLine("<div class='vista-titulo'>零件 - 等轴测视图</div>");
                    html.AppendLine(GerarDiagramaBloco(pecaL, pecaW, pecaH, "peca"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>长度: {0:F0}mm &nbsp;|&nbsp; 宽度: {1:F0}mm &nbsp;|&nbsp; 高度: {2:F0}mm</div>", pecaL, pecaW, pecaH));
                    html.AppendLine("</div>");
                }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>夹具中的零件 - 等轴测视图</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, '夹具中的零件')\" style='cursor:pointer; width:100%; height:500px; object-fit:fill;' src='data:image/jpeg;base64,{0}' alt='装夹在虎钳中的零件'/>", imagemPrincipal));
                html.AppendLine("</div>");
            }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>配置（夹具）</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Vista Principal')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='带夹具的主等轴测视图'/>", imagemPrincipal));
                html.AppendLine("</div>");

                if (!string.IsNullOrEmpty(imagemOrigemWCS))
                {
                    html.AppendLine("<div class='vista-setup vista-principal setup-instrucoes-imagem-horizontal'>");
                    html.AppendLine("<div class='vista-titulo'>工件坐标原点 (WCS / G54) - 俯视图</div>");
                    html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Origem WCS')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='WCS工件原点'/>", imagemOrigemWCS));
                    html.AppendLine("</div>");
                }
            ...
            html.AppendLine("<tr><th>T#</th><th>名称</th><th class='num'>直径</th><th class='num'>长度</th></tr>");
            ...
            // reduz a altura total pela metade quando ha muitas 道工序.
            int totalOps = 道工序Ordenadas.Count;
            int metade = (totalOps + 1) / 2;
            List<OperacaoOrdenada> primeiraMetade = 道工序Ordenadas.GetRange(0, Math.Min(metade, totalOps));
            List<OperacaoOrdenada> segundaMetade = totalOps > metade
                ? 道工序Ordenadas.GetRange(metade, totalOps - metade)
            ...
            html.AppendLine(string.Format(
                "<div class='resumo'>总计 &nbsp;&nbsp;|&nbsp;&nbsp; 切削时间: {0} &nbsp;&nbsp;|&nbsp;&nbsp; 总时间: {1}</div>",
            ...
            // ================================================================
            html.AppendLine("<div class='pagina-impressao'>");
            html.AppendLine("<h2>刀具详情</h2>");
            html.AppendLine("<div class='vistas-container'>");

            foreach (KeyValuePair<string, DadosFerramenta> kv in ferramentas)
            {
                DadosFerramenta f = kv.Value;
                string 道工序Lista = string.Join(", ", f.Operacoes.ToArray());
                ...
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd' colspan='2'>Operacao: {0}</td></tr>", 道工序Lista));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>刀具编号：</td><td class='td-valor-fd'>{0}</td></tr>", f.NumeroFerramenta >= 0 ? f.NumeroFerramenta.ToString() : "-"));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>刀具名称：</td><td class='td-valor-fd'>{0}</td></tr>", f.Nome));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>刀具直径：</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.Diametro)));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>刀具自由长度：</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.ComprimentoCorte)));
                ...
                if (temBloco && f.ComprimentoCorte > 0)
                {
                    ...
                    string textoAlerta = alcancaProfundidade
                        ? string.Format("正常 - 可达到 {0:F0}毫米 毛坯深度 ({1:F0}mm)", blocoH, f.ComprimentoCorte)
                        : string.Format("警告 - 毛坯深度 ({0:F0}mm) 大于刀具可达范围 ({1:F0}mm)", blocoH, f.ComprimentoCorte);

                    html.AppendLine(string.Format(
                        "<tr><td class='td-rotulo-fd'>最小伸出长度（估计）：</td><td class='td-valor-fd' style='color:{0};'>{1}</td></tr>",
            ...
            // quando ha muitas 道工序.
            // ================================================================
            List<OperacaoOrdenada> 道工序ComImagemIPW = new List<OperacaoOrdenada>();
            foreach (OperacaoOrdenada op in 道工序Ordenadas)
            {
                if (!string.IsNullOrEmpty(op.ImagemIPW)) 道工序ComImagemIPW.Add(op);
            }

            const int imagensPorPagina = 4;
            int totalPaginasImagens = (int)Math.Ceiling(道工序ComImagemIPW.Count / (double)imagensPorPagina);
            ...
            for (int p = 0; p < totalPaginasImagens; p++)
            {
                ...
                int fim = Math.Min(inicio + imagensPorPagina, 道工序ComImagemIPW.Count);

                for (int i = inicio; i < fim; i++)
                {
                    OperacaoOrdenada op = 道工序ComImagemIPW[i];
        ...
        private static string GerarTabelaOperacoes(List<OperacaoOrdenada> lista)
        {
            ...
            sb.AppendLine("<tr><th>Img</th><th>#</th><th>工序</th><th>刀具</th><th class='num'>切削时间</th><th class='num'>总时间</th></tr>");
