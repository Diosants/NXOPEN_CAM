...
{
    public class SHOP_DOCUMENTATION_FR
    {
        // ================================================================
        // >>> PERSONALIZACAO POR CLIENT - EDITE AQUI <<<
        //
        ...
        private const string TITULO_DOCUMENTO = "FICHE DE PROCESSUS";
        ...
        public static void Run(string[] args)
        {
            ...
            // ----------------------------------------------------------------
            // 1) Percorre as opérations, agrupando tudo por FERRAMENTA
            // ----------------------------------------------------------------
            Dictionary<string, DadosFerramenta> ferramentas = new Dictionary<string, DadosFerramenta>();
            List<OperacaoOrdenada> opérationsOrdenadas = new List<OperacaoOrdenada>();
            ...
            foreach (CAMObject camObj in workPart.CAMSetup.CAMOperationCollection)
            {
                ...
                // real da colecao de opérations do CAM), independente de
                // ter conseguido achar a ferramenta ou nao.
                opérationsOrdenadas.Add(new OperacaoOrdenada
                {
            ...
            html.AppendLine(string.Format("<div class='info'>{0} &nbsp;&nbsp;|&nbsp;&nbsp; Généré le {1} &nbsp;&nbsp;|&nbsp;&nbsp; {2} opérations &nbsp;&nbsp;|&nbsp;&nbsp; {3} outils</div>",
            ...
            html.AppendLine(LinhaInfoEditavel("PROGRAMMEUR", programador));
            html.AppendLine(LinhaInfoEditavel("ENTREPRISE", ""));
            html.AppendLine(LinhaInfoEditavel("CLIENT", ""));
            html.AppendLine(LinhaInfoEditavel("MACHINE", ""));
            html.AppendLine(LinhaInfoEditavel("COMMANDE", ""));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<table class='tabela-info'>");
            html.AppendLine(LinhaInfoEditavel("ORDRE DE FABRICATION", "Nº"));
            html.AppendLine(LinhaInfoEditavel("DATE", dataGeracao.Split(' ')[0]));
            html.AppendLine(LinhaInfoEditavel("DÉLAI DE LIVRAISON", ""));
            html.AppendLine(LinhaInfoEditavel("QTÉ. PIÈCES", "Nº"));
            html.AppendLine(LinhaInfoEditavel("OPÉRATEUR", "NOM:"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            html.AppendLine("<div class='coluna-esquerda-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>DIMENSIONS DE LA MATIÈRE PREMIÈRE</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LONGUEUR", "X", temBloco ? blocoL.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("LARGEUR", "Y", temBloco ? blocoW.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("HAUTEUR/ÉPAISSEUR", "Z", temBloco ? blocoH.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>DIMENSIONS DE LA PIÈCE</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LONGUEUR", "X", temPeca ? pecaL.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("LARGEUR", "Y", temPeca ? pecaW.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("HAUTEUR/ÉPAISSEUR", "Z", temPeca ? pecaH.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            if (temBloco || temPeca)
            {
                ...
                if (temBloco)
                {
                    ...
                    html.AppendLine("<div class='vista-titulo'>Matière Première - Isométrique</div>");
                    html.AppendLine(GerarDiagramaBloco(blocoL, blocoW, blocoH, "materiaprima"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Longueur: {0:F0}mm &nbsp;|&nbsp; Largeur: {1:F0}mm &nbsp;|&nbsp; Hauteur: {2:F0}mm</div>", blocoL, blocoW, blocoH));
                    html.AppendLine("</div>");
                }
                ...
                {
                    html.AppendLine("<div class='vista-setup vista-dim'>");
                    html.AppendLine("<div class='vista-titulo'>Pièce - Isométrique</div>");
                    html.AppendLine(GerarDiagramaBloco(pecaL, pecaW, pecaH, "peca"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Longueur: {0:F0}mm &nbsp;|&nbsp; Largeur: {1:F0}mm &nbsp;|&nbsp; Hauteur: {2:F0}mm</div>", pecaL, pecaW, pecaH));
                    html.AppendLine("</div>");
                }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Pièce en Fixation - Vue Isométrique</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Pièce en Fixation')\" style='cursor:pointer; width:100%; height:500px; object-fit:fill;' src='data:image/jpeg;base64,{0}' alt='Pièce montée dans l'étau'/>", imagemPrincipal));
                html.AppendLine("</div>");
            }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Configuration (fixation)</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Vista Principal')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='Vue principale isométrique avec fixation'/>", imagemPrincipal));
                html.AppendLine("</div>");

                if (!string.IsNullOrEmpty(imagemOrigemWCS))
                {
                    html.AppendLine("<div class='vista-setup vista-principal setup-instrucoes-imagem-horizontal'>");
                    html.AppendLine("<div class='vista-titulo'>Origine Pièce (WCS / G54) - Vue de Dessus</div>");
                    html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Origem WCS')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='Origine de travail WCS'/>", imagemOrigemWCS));
                    html.AppendLine("</div>");
                }
            ...
            html.AppendLine("<tr><th>T#</th><th>Nom</th><th class='num'>Diamètre</th><th class='num'>Longueur</th></tr>");
            ...
            // reduz a altura total pela metade quando ha muitas opérations.
            int totalOps = opérationsOrdenadas.Count;
            int metade = (totalOps + 1) / 2;
            List<OperacaoOrdenada> primeiraMetade = opérationsOrdenadas.GetRange(0, Math.Min(metade, totalOps));
            List<OperacaoOrdenada> segundaMetade = totalOps > metade
                ? opérationsOrdenadas.GetRange(metade, totalOps - metade)
            ...
            html.AppendLine(string.Format(
                "<div class='resumo'>TOTAL GÉNÉRAL &nbsp;&nbsp;|&nbsp;&nbsp; Temps de coupe: {0} &nbsp;&nbsp;|&nbsp;&nbsp; Temps total: {1}</div>",
                FormatarTempo(tempoCorteTotalGeral), FormatarTempo(tempoTotalGeral)));

            html.AppendLine("</div>"); // fim pagina-impressao 3

            // ================================================================
            // PAGINA 4: OUTIL DÉTAILLÉ
            // ================================================================
            html.AppendLine("<div class='pagina-impressao'>");
            html.AppendLine("<h2>OUTIL DÉTAILLÉ</h2>");
            html.AppendLine("<div class='vistas-container'>");

            foreach (KeyValuePair<string, DadosFerramenta> kv in ferramentas)
            {
                DadosFerramenta f = kv.Value;
                string opérationsLista = string.Join(", ", f.Operacoes.ToArray());
                ...
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd' colspan='2'>Operacao: {0}</td></tr>", opérationsLista));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Numéro d'Outil:</td><td class='td-valor-fd'>{0}</td></tr>", f.NumeroFerramenta >= 0 ? f.NumeroFerramenta.ToString() : "-"));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Nom de l'Outil:</td><td class='td-valor-fd'>{0}</td></tr>", f.Nome));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Diamètre de l'Outil:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.Diametro)));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Longueur Libre de l'Outil:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.ComprimentoCorte)));
                ...
                if (temBloco && f.ComprimentoCorte > 0)
                {
                    ...
                    string textoAlerta = alcancaProfundidade
                        ? string.Format("OK - atteint {0:F0}mm de profondeur du bloc ({1:F0}mm)", blocoH, f.ComprimentoCorte)
                        : string.Format("ATTENTION - profondeur du bloc ({0:F0}mm) supérieure à la portée de l'outil ({1:F0}mm)", blocoH, f.ComprimentoCorte);

                    html.AppendLine(string.Format(
                        "<tr><td class='td-rotulo-fd'>Porte-à-faux Minimum (estimé):</td><td class='td-valor-fd' style='color:{0};'>{1}</td></tr>",
            ...
            // quando ha muitas opérations.
            // ================================================================
            List<OperacaoOrdenada> opérationsComImagemIPW = new List<OperacaoOrdenada>();
            foreach (OperacaoOrdenada op in opérationsOrdenadas)
            {
                if (!string.IsNullOrEmpty(op.ImagemIPW)) opérationsComImagemIPW.Add(op);
            }

            const int imagensPorPagina = 4;
            int totalPaginasImagens = (int)Math.Ceiling(opérationsComImagemIPW.Count / (double)imagensPorPagina);
            ...
            for (int p = 0; p < totalPaginasImagens; p++)
            {
                ...
                int fim = Math.Min(inicio + imagensPorPagina, opérationsComImagemIPW.Count);

                for (int i = inicio; i < fim; i++)
                {
                    OperacaoOrdenada op = opérationsComImagemIPW[i];
        ...
        private static string GerarTabelaOperacoes(List<OperacaoOrdenada> lista)
        {
            ...
            sb.AppendLine("<tr><th>Img</th><th>#</th><th>Opération</th><th>Outil</th><th class='num'>Temps de Coupe</th><th class='num'>Temps Total</th></tr>");
