...
{
    public class SHOP_DOCUMENTATION_NL
    {
        // ================================================================
        // >>> PERSONALIZACAO POR KLANT - EDITE AQUI <<<
        //
        ...
        private const string TITULO_DOCUMENTO = "PROCESBLAD";
        ...
        public static void Run(string[] args)
        {
            ...
            // ----------------------------------------------------------------
            // 1) Percorre as bewerkingen, agrupando tudo por FERRAMENTA
            // ----------------------------------------------------------------
            Dictionary<string, DadosFerramenta> ferramentas = new Dictionary<string, DadosFerramenta>();
            List<OperacaoOrdenada> bewerkingenOrdenadas = new List<OperacaoOrdenada>();
            ...
            foreach (CAMObject camObj in workPart.CAMSetup.CAMOperationCollection)
            {
                ...
                // real da colecao de bewerkingen do CAM), independente de
                // ter conseguido achar a ferramenta ou nao.
                bewerkingenOrdenadas.Add(new OperacaoOrdenada
                {
            ...
            html.AppendLine(string.Format("<div class='info'>{0} &nbsp;&nbsp;|&nbsp;&nbsp; Gegenereerd op {1} &nbsp;&nbsp;|&nbsp;&nbsp; {2} bewerkingen &nbsp;&nbsp;|&nbsp;&nbsp; {3} gereedschappen</div>",
            ...
            html.AppendLine(LinhaInfoEditavel("BEDRIJF", ""));
            html.AppendLine(LinhaInfoEditavel("KLANT", ""));
            html.AppendLine(LinhaInfoEditavel("MACHINE", ""));
            html.AppendLine(LinhaInfoEditavel("BESTURING", ""));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<table class='tabela-info'>");
            html.AppendLine(LinhaInfoEditavel("WERKORDER", "Nº"));
            html.AppendLine(LinhaInfoEditavel("DATUM", dataGeracao.Split(' ')[0]));
            html.AppendLine(LinhaInfoEditavel("LEVERTERMIJN", ""));
            html.AppendLine(LinhaInfoEditavel("AANTAL DELEN", "Nº"));
            html.AppendLine(LinhaInfoEditavel("OPERATOR", "NAAM:"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            html.AppendLine("<div class='coluna-esquerda-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>AFMETINGEN GRONDSTOF</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LENGTE", "X", temBloco ? blocoL.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("BREEDTE", "Y", temBloco ? blocoW.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("HOOGTE/DIKTE", "Z", temBloco ? blocoH.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>AFMETINGEN ONDERDEEL</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LENGTE", "X", temPeca ? pecaL.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("BREEDTE", "Y", temPeca ? pecaW.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("HOOGTE/DIKTE", "Z", temPeca ? pecaH.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            if (temBloco || temPeca)
            {
                ...
                if (temBloco)
                {
                    ...
                    html.AppendLine("<div class='vista-titulo'>Grondstof - Isometrisch</div>");
                    html.AppendLine(GerarDiagramaBloco(blocoL, blocoW, blocoH, "materiaprima"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Lengte: {0:F0}mm &nbsp;|&nbsp; Breedte: {1:F0}mm &nbsp;|&nbsp; Hoogte: {2:F0}mm</div>", blocoL, blocoW, blocoH));
                    html.AppendLine("</div>");
                }
                ...
                {
                    html.AppendLine("<div class='vista-setup vista-dim'>");
                    html.AppendLine("<div class='vista-titulo'>Onderdeel - Isometrisch</div>");
                    html.AppendLine(GerarDiagramaBloco(pecaL, pecaW, pecaH, "peca"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Lengte: {0:F0}mm &nbsp;|&nbsp; Breedte: {1:F0}mm &nbsp;|&nbsp; Hoogte: {2:F0}mm</div>", pecaL, pecaW, pecaH));
                    html.AppendLine("</div>");
                }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Onderdeel in Opspanning - Isometrisch Aanzicht</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Onderdeel in Opspanning')\" style='cursor:pointer; width:100%; height:500px; object-fit:fill;' src='data:image/jpeg;base64,{0}' alt='Onderdeel gemonteerd in bankschroef'/>", imagemPrincipal));
                html.AppendLine("</div>");
            }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Configuratie (opspanning)</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Vista Principal')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='Isometrisch hoofdaanzicht met opspanning'/>", imagemPrincipal));
                html.AppendLine("</div>");

                if (!string.IsNullOrEmpty(imagemOrigemWCS))
                {
                    html.AppendLine("<div class='vista-setup vista-principal setup-instrucoes-imagem-horizontal'>");
                    html.AppendLine("<div class='vista-titulo'>Werkstuknulpunt (WCS / G54) - Bovenaanzicht</div>");
                    html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Origem WCS')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='WCS werkstuknulpunt'/>", imagemOrigemWCS));
                    html.AppendLine("</div>");
                }
            ...
            html.AppendLine("<tr><th>T#</th><th>Naam</th><th class='num'>Diameter</th><th class='num'>Lengte</th></tr>");
            ...
            // reduz a altura total pela metade quando ha muitas bewerkingen.
            int totalOps = bewerkingenOrdenadas.Count;
            int metade = (totalOps + 1) / 2;
            List<OperacaoOrdenada> primeiraMetade = bewerkingenOrdenadas.GetRange(0, Math.Min(metade, totalOps));
            List<OperacaoOrdenada> segundaMetade = totalOps > metade
                ? bewerkingenOrdenadas.GetRange(metade, totalOps - metade)
            ...
            html.AppendLine(string.Format(
                "<div class='resumo'>TOTAAL GENERAAL &nbsp;&nbsp;|&nbsp;&nbsp; Snijtijd: {0} &nbsp;&nbsp;|&nbsp;&nbsp; Totale tijd: {1}</div>",
                FormatarTempo(tempoCorteTotalGeral), FormatarTempo(tempoTotalGeral)));

            html.AppendLine("</div>"); // fim pagina-impressao 3

            // ================================================================
            // PAGINA 4: GEREEDSCHAP DETAIL
            // ================================================================
            html.AppendLine("<div class='pagina-impressao'>");
            html.AppendLine("<h2>GEREEDSCHAP DETAIL</h2>");
            html.AppendLine("<div class='vistas-container'>");

            foreach (KeyValuePair<string, DadosFerramenta> kv in ferramentas)
            {
                DadosFerramenta f = kv.Value;
                string bewerkingenLista = string.Join(", ", f.Operacoes.ToArray());
                ...
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd' colspan='2'>Operacao: {0}</td></tr>", bewerkingenLista));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Gereedschapnummer:</td><td class='td-valor-fd'>{0}</td></tr>", f.NumeroFerramenta >= 0 ? f.NumeroFerramenta.ToString() : "-"));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Gereedschapnaam:</td><td class='td-valor-fd'>{0}</td></tr>", f.Nome));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Gereedschapdiameter:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.Diametro)));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Vrije Gereedschapslengte:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.ComprimentoCorte)));
                ...
                if (temBloco && f.ComprimentoCorte > 0)
                {
                    ...
                    string textoAlerta = alcancaProfundidade
                        ? string.Format("OK - bereikt {0:F0}mm blokdiepte ({1:F0}mm)", blocoH, f.ComprimentoCorte)
                        : string.Format("LET OP - blokdiepte ({0:F0}mm) groter dan het bereik van het gereedschap ({1:F0}mm)", blocoH, f.ComprimentoCorte);

                    html.AppendLine(string.Format(
                        "<tr><td class='td-rotulo-fd'>Minimale Uitsteeklengte (geschat):</td><td class='td-valor-fd' style='color:{0};'>{1}</td></tr>",
            ...
            // quando ha muitas bewerkingen.
            // ================================================================
            List<OperacaoOrdenada> bewerkingenComImagemIPW = new List<OperacaoOrdenada>();
            foreach (OperacaoOrdenada op in bewerkingenOrdenadas)
            {
                if (!string.IsNullOrEmpty(op.ImagemIPW)) bewerkingenComImagemIPW.Add(op);
            }

            const int imagensPorPagina = 4;
            int totalPaginasImagens = (int)Math.Ceiling(bewerkingenComImagemIPW.Count / (double)imagensPorPagina);
            ...
            for (int p = 0; p < totalPaginasImagens; p++)
            {
                ...
                int fim = Math.Min(inicio + imagensPorPagina, bewerkingenComImagemIPW.Count);

                for (int i = inicio; i < fim; i++)
                {
                    OperacaoOrdenada op = bewerkingenComImagemIPW[i];
        ...
        private static string GerarTabelaOperacoes(List<OperacaoOrdenada> lista)
        {
            ...
            sb.AppendLine("<tr><th>Img</th><th>#</th><th>Bewerking</th><th>Gereedschap</th><th class='num'>Snijtijd</th><th class='num'>Totale Tijd</th></tr>");
