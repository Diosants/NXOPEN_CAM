...
// CADCAMWise - Automatic Setup Sheet for NX CAM
// ============================================================================
//
// WHAT THIS JOURNAL DOES:
// Generates a complete "Setup Sheet" (Process Sheet) in HTML, from the
// CAM data of the currently open part in NX: tools used (with 3D-like
// technical drawings), operation times, raw material block and final
// part dimensions, isometric views (with fixture and WCS origin), and
// machining progress images (IPW) per operation. The generated HTML is
// a single, self-contained file (no external image dependencies), with
// editable fields (contenteditable) for the programmer to fill in
// before printing, and a layout ready for A4 landscape printing (5+
// pages with automatic page breaks).
//
// ----------------------------------------------------------------------
// PREREQUISITES / ASSUMPTIONS (the script may fail or produce
// incomplete data if these aren't true for your part):
// ----------------------------------------------------------------------
//   1. The part needs a valid CAMSetup (CAM module licensed on the NX
//      instance running the journal).
//   2. There is a geometry group named exactly "WORKPIECE" (used to
//      read the block/Blank dimensions). If the name is different,
//      block dimension extraction fails silently (warning in the log,
//      but the rest of the document keeps working).
//   3. The block (raw material) and the final part are 2 SEPARATE
//      SOLID BODIES in the file (not a single body serving both
//      purposes). The part is identified by elimination: the body
//      whose dimensions do NOT match the block's.
//   4. DRILL-type tools that don't expose all data via the API rely on
//      NAME CONVENTION for diameter (e.g., "HSS_DRILL_D25",
//      "DRILL_DIAM_16MM", "CBORE_24MM" - number in the name, with or
//      without "MM" suffix). Tools with no number in the name (e.g.,
//      "CENTER_DRILL" alone) get generic estimated diameter/length.
//   5. The fixture (vise/clamps) needs to be visible on screen when
//      running, to appear in the "Configuration" views.
//
// ----------------------------------------------------------------------
// REAL CONFIRMED APIs (via journal recorded by the user or tested
// official documentation - not guesses; each one was validated running
// without error on a real part). Useful for reuse in other NXOpen
// automation projects:
// ----------------------------------------------------------------------
//   - operacao.GetToolpathCuttingTime() / GetToolpathTime()
//     WARNING: official documentation says "returns in seconds", but
//     real-world tests confirmed it returns in MINUTES (60x factor
//     confirmed by comparing with Information->Object). Fixed in code
//     by multiplying by 60.
//
//   - operacao.GetParent(CAMSetup.View.MachineTool) -> NCGroup
//     Returns the parent group in the tool view. If it's of type
//     NXOpen.CAM.Tool, it's the operation's real tool.
//
//   - workPart.CAMSetup.CAMGroupCollection.CreateMillToolBuilder(tool)
//     -> MillToolBuilder, with:
//       .TlDiameterBuilder.Value      (diameter)
//       .TlCor1RadBuilder.Value       (corner radius)
//       .TlFluteLnBuilder.Value       (cutting/flute length)
//       .TlHeightBuilder.Value        (total length)
//       .TlNumFlutesBuilder.Value     (number of flutes)
//       .TlNumberBuilder.Value        (tool number)
//       .TlTaperAngBuilder.Value      (taper angle)
//       .TlTipAngBuilder.Value        (tip angle - does NOT exist on
//                                      DrillStdToolBuilder, only Mill)
//       .HolderSectionBuilder.GetSection(i) +
//         .GetAllParameters(section, out lowerD, out length, out
//                            taperAng, out upperD, out cornerRad)
//         -> real tool holder sections, however many exist
//            (index 0, 1, 2...)
//
//   - For DRILL-type tools (the MillToolBuilder above fails or doesn't
//     expose the properties), specific builders exist, with the SAME
//     property names as MillToolBuilder (TlNumberBuilder etc), just
//     changing the class/creation method name:
...
//     (a builder for "Boring Bar" tools also exists - the cast error
//     showed the tool's real type as "NXOpen.CAM.DrillBoringBarTool",
//     but the exact BUILDER name for that type hasn't been confirmed
//     yet - would need another journal recording)
//
//   - workPart.CAMSetup.CAMGroupCollection.FindObject("WORKPIECE")
//     -> NXOpen.CAM.FeatureGeometry, used with:
//     CreateMillGeomBuilder(workpiece).BlankGeometry.BlockLength /
//     .BlockWidth / .BlockHeight
//     -> raw material block dimensions, when defined as a parametric
//        block (not a separate solid body)
//
//   - theSession.CAMSession.PathDisplay.ShowToolPath(operation) /
//     .HideToolPath(operation) -> shows/hides the toolpath LINE of a
//     specific operation on screen (does not show the solid tool)
//
//   - workPart.CAMSetup.Show3dWorkpiece(new CAMObject[]{operation}) /
//     .Delete3dWorkpieces(...) -> shows/deletes the IPW (In-Process
//     Workpiece) - the REAL material stage after that operation. Works
//     well, but has a computational cost (slows down generation with
//     many operations).
//
//   - theSession.DisplayManager.BlankObjects(objects) /
//     .UnblankObjects(objects) -> hides/shows objects (Body,
//     Component) on screen. Objects must be DisplayableObject[].
//     CAUTION: hiding ALL assembly components can hide the part/block
//     itself if they're inside a component - already tried this and
//     it broke (blank image). We only hide Body objects for now, not
//     Component.
//
//   - UFSession.GetUFSession().Disp.CreateImage(path, format, color)
//     -> takes a "screenshot" of the current screen and saves it as an
//     image file. Confirmed and working API (shows as "Deprecated" in
//     the signature, but that's just a warning, doesn't prevent
//     compiling/running).
//
//   - workPart.WCS.Visibility = true/false -> controls visibility of
//     the origin triad (WCS) on screen.
//
// ----------------------------------------------------------------------
// ATTEMPTS THAT DIDN'T WORK (documented to avoid repeating the same
// mistake):
// ----------------------------------------------------------------------
//   - Showing the SOLID tool (not just the line) during ShowToolPath:
//     we tried NXOpen.CAM.OperationDisplayOptionsBuilder (doesn't exist
//     the way the documentation suggested), then
//     PathDisplay.SetToolDisplayType(SolidWithHolder) alone (no
//     effect), then SetToolDisplayType + PathDisplay.Play(true) + a
//     300ms pause (still no effect). Hypothesis: Play() only actually
//     renders in an interactive session (with an active screen event
//     loop), not working via journal/batch. No solution found so far.
//
//   - CAMSetup.MinToolLen(objects) to get the minimum tool length
//     calculated by NX: confirmed via official documentation that the
//     method returns VOID - it only triggers the on-screen display
//     (like the Information window), without returning the value to
//     the code. No programmatic use possible via this path.
//
// ============================================================================
...
{
    public class SHOP_DOCUMENTATION_EN
    {
        // ================================================================
        // >>> PERSONALIZACAO POR CUSTOMER - EDITE AQUI <<<
        //
        ...
        private const string TITULO_DOCUMENTO = "PROCESS SHEET";
        ...
        public static void Run(string[] args)
        {
            ...
            // ----------------------------------------------------------------
            // 1) Percorre as operations, agrupando tudo por FERRAMENTA
            // ----------------------------------------------------------------
            Dictionary<string, DadosFerramenta> ferramentas = new Dictionary<string, DadosFerramenta>();
            List<OperacaoOrdenada> operationsOrdenadas = new List<OperacaoOrdenada>();
            ...
            foreach (CAMObject camObj in workPart.CAMSetup.CAMOperationCollection)
            {
                ...
                // real da colecao de operations do CAM), independente de
                // ter conseguido achar a ferramenta ou nao.
                operationsOrdenadas.Add(new OperacaoOrdenada
                {
            ...
            html.AppendLine(string.Format("<div class='info'>{0} &nbsp;&nbsp;|&nbsp;&nbsp; Generated on {1} &nbsp;&nbsp;|&nbsp;&nbsp; {2} operations &nbsp;&nbsp;|&nbsp;&nbsp; {3} tools</div>",
            ...
            html.AppendLine(LinhaInfoEditavel("PROGRAMMER", programador));
            html.AppendLine(LinhaInfoEditavel("COMPANY", ""));
            html.AppendLine(LinhaInfoEditavel("CUSTOMER", ""));
            html.AppendLine(LinhaInfoEditavel("MACHINE", ""));
            html.AppendLine(LinhaInfoEditavel("CONTROL", ""));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<table class='tabela-info'>");
            html.AppendLine(LinhaInfoEditavel("WORK ORDER", "No."));
            html.AppendLine(LinhaInfoEditavel("DATE", dataGeracao.Split(' ')[0]));
            html.AppendLine(LinhaInfoEditavel("DELIVERY DEADLINE", ""));
            html.AppendLine(LinhaInfoEditavel("QTY. PARTS", "No."));
            html.AppendLine(LinhaInfoEditavel("OPERATOR", "NAME:"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            html.AppendLine("<div class='coluna-esquerda-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>RAW MATERIAL DIMENSIONS</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LENGTH", "X", temBloco ? blocoL.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("WIDTH", "Y", temBloco ? blocoW.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("HEIGHT/THICKNESS", "Z", temBloco ? blocoH.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>PART DIMENSIONS</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LENGTH", "X", temPeca ? pecaL.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("WIDTH", "Y", temPeca ? pecaW.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("HEIGHT/THICKNESS", "Z", temPeca ? pecaH.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            if (temBloco || temPeca)
            {
                ...
                if (temBloco)
                {
                    ...
                    html.AppendLine("<div class='vista-titulo'>Raw Material - Isometric</div>");
                    html.AppendLine(GerarDiagramaBloco(blocoL, blocoW, blocoH, "materiaprima"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Length: {0:F0}mm &nbsp;|&nbsp; Width: {1:F0}mm &nbsp;|&nbsp; Height: {2:F0}mm</div>", blocoL, blocoW, blocoH));
                    html.AppendLine("</div>");
                }
                ...
                {
                    html.AppendLine("<div class='vista-setup vista-dim'>");
                    html.AppendLine("<div class='vista-titulo'>Part - Isometric</div>");
                    html.AppendLine(GerarDiagramaBloco(pecaL, pecaW, pecaH, "peca"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Length: {0:F0}mm &nbsp;|&nbsp; Width: {1:F0}mm &nbsp;|&nbsp; Height: {2:F0}mm</div>", pecaL, pecaW, pecaH));
                    html.AppendLine("</div>");
                }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Part in Fixture - Isometric View</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Part in Fixture')\" style='cursor:pointer; width:100%; height:500px; object-fit:fill;' src='data:image/jpeg;base64,{0}' alt='Part mounted in vise'/>", imagemPrincipal));
                html.AppendLine("</div>");
            }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Configuration (fixture)</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Vista Principal')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='Main isometric view with fixture'/>", imagemPrincipal));
                html.AppendLine("</div>");

                if (!string.IsNullOrEmpty(imagemOrigemWCS))
                {
                    html.AppendLine("<div class='vista-setup vista-principal setup-instrucoes-imagem-horizontal'>");
                    html.AppendLine("<div class='vista-titulo'>Work Origin (WCS / G54) - Top View</div>");
                    html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Origem WCS')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='WCS work origin'/>", imagemOrigemWCS));
                    html.AppendLine("</div>");
                }
            ...
            html.AppendLine("<tr><th>T#</th><th>Name</th><th class='num'>Diameter</th><th class='num'>Length</th></tr>");
            ...
            // reduz a altura total pela metade quando ha muitas operations.
            int totalOps = operationsOrdenadas.Count;
            int metade = (totalOps + 1) / 2;
            List<OperacaoOrdenada> primeiraMetade = operationsOrdenadas.GetRange(0, Math.Min(metade, totalOps));
            List<OperacaoOrdenada> segundaMetade = totalOps > metade
                ? operationsOrdenadas.GetRange(metade, totalOps - metade)
            ...
            html.AppendLine(string.Format(
                "<div class='resumo'>GRAND TOTAL &nbsp;&nbsp;|&nbsp;&nbsp; Cutting time: {0} &nbsp;&nbsp;|&nbsp;&nbsp; Total time: {1}</div>",
                FormatarTempo(tempoCorteTotalGeral), FormatarTempo(tempoTotalGeral)));

            html.AppendLine("</div>"); // fim pagina-impressao 3

            // ================================================================
            // PAGINA 4: TOOL DETAIL
            // ================================================================
            html.AppendLine("<div class='pagina-impressao'>");
            html.AppendLine("<h2>TOOL DETAIL</h2>");
            html.AppendLine("<div class='vistas-container'>");

            foreach (KeyValuePair<string, DadosFerramenta> kv in ferramentas)
            {
                DadosFerramenta f = kv.Value;
                string operationsLista = string.Join(", ", f.Operacoes.ToArray());
                ...
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd' colspan='2'>Operacao: {0}</td></tr>", operationsLista));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Tool Number:</td><td class='td-valor-fd'>{0}</td></tr>", f.NumeroFerramenta >= 0 ? f.NumeroFerramenta.ToString() : "-"));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Tool Name:</td><td class='td-valor-fd'>{0}</td></tr>", f.Nome));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Tool Diameter:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.Diametro)));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Tool Free Length:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.ComprimentoCorte)));
                ...
                if (temBloco && f.ComprimentoCorte > 0)
                {
                    ...
                    string textoAlerta = alcancaProfundidade
                        ? string.Format("OK - reaches {0:F0}mm of block depth ({1:F0}mm)", blocoH, f.ComprimentoCorte)
                        : string.Format("WARNING - block depth ({0:F0}mm) greater than tool reach ({1:F0}mm)", blocoH, f.ComprimentoCorte);

                    html.AppendLine(string.Format(
                        "<tr><td class='td-rotulo-fd'>Minimum Stickout (estimated):</td><td class='td-valor-fd' style='color:{0};'>{1}</td></tr>",
            ...
            // quando ha muitas operations.
            // ================================================================
            List<OperacaoOrdenada> operationsComImagemIPW = new List<OperacaoOrdenada>();
            foreach (OperacaoOrdenada op in operationsOrdenadas)
            {
                if (!string.IsNullOrEmpty(op.ImagemIPW)) operationsComImagemIPW.Add(op);
            }

            const int imagensPorPagina = 4;
            int totalPaginasImagens = (int)Math.Ceiling(operationsComImagemIPW.Count / (double)imagensPorPagina);
            ...
            for (int p = 0; p < totalPaginasImagens; p++)
            {
                ...
                int fim = Math.Min(inicio + imagensPorPagina, operationsComImagemIPW.Count);

                for (int i = inicio; i < fim; i++)
                {
                    OperacaoOrdenada op = operationsComImagemIPW[i];
        ...
        private static string GerarTabelaOperacoes(List<OperacaoOrdenada> lista)
        {
            ...
            sb.AppendLine("<tr><th>Img</th><th>#</th><th>Operation</th><th>Tool</th><th class='num'>Cutting Time</th><th class='num'>Total Time</th></tr>");
