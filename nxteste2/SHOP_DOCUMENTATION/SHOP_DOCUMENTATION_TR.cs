...
{
    public class SHOP_DOCUMENTATION_TR
    {
        // ================================================================
        // >>> PERSONALIZACAO POR MÜŞTERİ - EDITE AQUI <<<
        //
        ...
        private const string TITULO_DOCUMENTO = "PROSES SAYFASI";
        ...
        public static void Run(string[] args)
        {
            ...
            // ----------------------------------------------------------------
            // 1) Percorre as operasyon, agrupando tudo por FERRAMENTA
            // ----------------------------------------------------------------
            Dictionary<string, DadosFerramenta> ferramentas = new Dictionary<string, DadosFerramenta>();
            List<OperacaoOrdenada> operasyonOrdenadas = new List<OperacaoOrdenada>();
            ...
            foreach (CAMObject camObj in workPart.CAMSetup.CAMOperationCollection)
            {
                ...
                // real da colecao de operasyon do CAM), independente de
                // ter conseguido achar a ferramenta ou nao.
                operasyonOrdenadas.Add(new OperacaoOrdenada
                {
            ...
            html.AppendLine(string.Format("<div class='info'>{0} &nbsp;&nbsp;|&nbsp;&nbsp; Oluşturulma tarihi {1} &nbsp;&nbsp;|&nbsp;&nbsp; {2} operasyon &nbsp;&nbsp;|&nbsp;&nbsp; {3} takım</div>",
            ...
            html.AppendLine(LinhaInfoEditavel("PROGRAMCI", programador));
            html.AppendLine(LinhaInfoEditavel("FİRMA", ""));
            html.AppendLine(LinhaInfoEditavel("MÜŞTERİ", ""));
            html.AppendLine(LinhaInfoEditavel("MAKİNE", ""));
            html.AppendLine(LinhaInfoEditavel("KONTROL", ""));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<table class='tabela-info'>");
            html.AppendLine(LinhaInfoEditavel("İŞ EMRİ", "Nº"));
            html.AppendLine(LinhaInfoEditavel("TARİH", dataGeracao.Split(' ')[0]));
            html.AppendLine(LinhaInfoEditavel("TESLİM SÜRESİ", ""));
            html.AppendLine(LinhaInfoEditavel("PARÇA ADEDİ", "Nº"));
            html.AppendLine(LinhaInfoEditavel("OPERATÖR", "AD:"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            html.AppendLine("<div class='coluna-esquerda-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>HAM MADDE BOYUTLARI</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("UZUNLUK", "X", temBloco ? blocoL.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("GENİŞLİK", "Y", temBloco ? blocoW.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("YÜKSEKLİK/KALINLIK", "Z", temBloco ? blocoH.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>PARÇA BOYUTLARI</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("UZUNLUK", "X", temPeca ? pecaL.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("GENİŞLİK", "Y", temPeca ? pecaW.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("YÜKSEKLİK/KALINLIK", "Z", temPeca ? pecaH.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            if (temBloco || temPeca)
            {
                ...
                if (temBloco)
                {
                    ...
                    html.AppendLine("<div class='vista-titulo'>Ham Madde - İzometrik</div>");
                    html.AppendLine(GerarDiagramaBloco(blocoL, blocoW, blocoH, "materiaprima"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Uzunluk: {0:F0}mm &nbsp;|&nbsp; Genişlik: {1:F0}mm &nbsp;|&nbsp; Yükseklik: {2:F0}mm</div>", blocoL, blocoW, blocoH));
                    html.AppendLine("</div>");
                }
                ...
                {
                    html.AppendLine("<div class='vista-setup vista-dim'>");
                    html.AppendLine("<div class='vista-titulo'>Parça - İzometrik</div>");
                    html.AppendLine(GerarDiagramaBloco(pecaL, pecaW, pecaH, "peca"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Uzunluk: {0:F0}mm &nbsp;|&nbsp; Genişlik: {1:F0}mm &nbsp;|&nbsp; Yükseklik: {2:F0}mm</div>", pecaL, pecaW, pecaH));
                    html.AppendLine("</div>");
                }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Bağlama Aparatındaki Parça - İzometrik Görünüm</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Bağlama Aparatındaki Parça')\" style='cursor:pointer; width:100%; height:500px; object-fit:fill;' src='data:image/jpeg;base64,{0}' alt='Mengeneye bağlanmış parça'/>", imagemPrincipal));
                html.AppendLine("</div>");
            }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Konfigürasyon (bağlama)</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Vista Principal')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='Bağlama aparatlı ana izometrik görünüm'/>", imagemPrincipal));
                html.AppendLine("</div>");

                if (!string.IsNullOrEmpty(imagemOrigemWCS))
                {
                    html.AppendLine("<div class='vista-setup vista-principal setup-instrucoes-imagem-horizontal'>");
                    html.AppendLine("<div class='vista-titulo'>İş Sıfır Noktası (WCS / G54) - Üstten Görünüm</div>");
                    html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Origem WCS')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='WCS iş sıfır noktası'/>", imagemOrigemWCS));
                    html.AppendLine("</div>");
                }
            ...
            html.AppendLine("<tr><th>T#</th><th>Ad</th><th class='num'>Çap</th><th class='num'>Uzunluk</th></tr>");
            ...
            // reduz a altura total pela metade quando ha muitas operasyon.
            int totalOps = operasyonOrdenadas.Count;
            int metade = (totalOps + 1) / 2;
            List<OperacaoOrdenada> primeiraMetade = operasyonOrdenadas.GetRange(0, Math.Min(metade, totalOps));
            List<OperacaoOrdenada> segundaMetade = totalOps > metade
                ? operasyonOrdenadas.GetRange(metade, totalOps - metade)
            ...
            html.AppendLine(string.Format(
                "<div class='resumo'>GENEL TOPLAM &nbsp;&nbsp;|&nbsp;&nbsp; Kesme süresi: {0} &nbsp;&nbsp;|&nbsp;&nbsp; Toplam süre: {1}</div>",
                FormatarTempo(tempoCorteTotalGeral), FormatarTempo(tempoTotalGeral)));

            html.AppendLine("</div>"); // fim pagina-impressao 3

            // ================================================================
            // PAGINA 4: DETAYLI TAKIM BİLGİSİ
            // ================================================================
            html.AppendLine("<div class='pagina-impressao'>");
            html.AppendLine("<h2>DETAYLI TAKIM BİLGİSİ</h2>");
            html.AppendLine("<div class='vistas-container'>");

            foreach (KeyValuePair<string, DadosFerramenta> kv in ferramentas)
            {
                DadosFerramenta f = kv.Value;
                string operasyonLista = string.Join(", ", f.Operacoes.ToArray());
                ...
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd' colspan='2'>Operacao: {0}</td></tr>", operasyonLista));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Takım Numarası:</td><td class='td-valor-fd'>{0}</td></tr>", f.NumeroFerramenta >= 0 ? f.NumeroFerramenta.ToString() : "-"));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Takım Adı:</td><td class='td-valor-fd'>{0}</td></tr>", f.Nome));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Takım Çapı:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.Diametro)));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Takım Serbest Boyu:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.ComprimentoCorte)));
                ...
                if (temBloco && f.ComprimentoCorte > 0)
                {
                    ...
                    string textoAlerta = alcancaProfundidade
                        ? string.Format("TAMAM - ulaşıyor {0:F0}mm blok derinliği ({1:F0}mm)", blocoH, f.ComprimentoCorte)
                        : string.Format("DİKKAT - blok derinliği ({0:F0}mm) takım erişiminden büyük ({1:F0}mm)", blocoH, f.ComprimentoCorte);

                    html.AppendLine(string.Format(
                        "<tr><td class='td-rotulo-fd'>Minimum Çıkıntı Uzunluğu (tahmini):</td><td class='td-valor-fd' style='color:{0};'>{1}</td></tr>",
            ...
            // quando ha muitas operasyon.
            // ================================================================
            List<OperacaoOrdenada> operasyonComImagemIPW = new List<OperacaoOrdenada>();
            foreach (OperacaoOrdenada op in operasyonOrdenadas)
            {
                if (!string.IsNullOrEmpty(op.ImagemIPW)) operasyonComImagemIPW.Add(op);
            }

            const int imagensPorPagina = 4;
            int totalPaginasImagens = (int)Math.Ceiling(operasyonComImagemIPW.Count / (double)imagensPorPagina);
            ...
            for (int p = 0; p < totalPaginasImagens; p++)
            {
                ...
                int fim = Math.Min(inicio + imagensPorPagina, operasyonComImagemIPW.Count);

                for (int i = inicio; i < fim; i++)
                {
                    OperacaoOrdenada op = operasyonComImagemIPW[i];
        ...
        private static string GerarTabelaOperacoes(List<OperacaoOrdenada> lista)
        {
            ...
            sb.AppendLine("<tr><th>Img</th><th>#</th><th>Operasyon</th><th>Takım</th><th class='num'>Kesme Süresi</th><th class='num'>Toplam Süre</th></tr>");
