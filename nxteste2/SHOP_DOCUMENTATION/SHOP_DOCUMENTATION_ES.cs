...
{
    public class SHOP_DOCUMENTATION_ES
    {
        // ================================================================
        // >>> PERSONALIZACAO POR CLIENTE - EDITE AQUI <<<
        //
        ...
        private const string TITULO_DOCUMENTO = "HOJA DE PROCESO";
        ...
        public static void Run(string[] args)
        {
            ...
            // ----------------------------------------------------------------
            // 1) Percorre as operaciones, agrupando tudo por FERRAMENTA
            // ----------------------------------------------------------------
            Dictionary<string, DadosFerramenta> ferramentas = new Dictionary<string, DadosFerramenta>();
            List<OperacaoOrdenada> operacionesOrdenadas = new List<OperacaoOrdenada>();
            ...
            foreach (CAMObject camObj in workPart.CAMSetup.CAMOperationCollection)
            {
                ...
                // real da colecao de operaciones do CAM), independente de
                // ter conseguido achar a ferramenta ou nao.
                operacionesOrdenadas.Add(new OperacaoOrdenada
                {
            ...
            html.AppendLine(string.Format("<div class='info'>{0} &nbsp;&nbsp;|&nbsp;&nbsp; Generado el {1} &nbsp;&nbsp;|&nbsp;&nbsp; {2} operaciones &nbsp;&nbsp;|&nbsp;&nbsp; {3} herramientas</div>",
            ...
            html.AppendLine(LinhaInfoEditavel("PROGRAMADOR", programador));
            html.AppendLine(LinhaInfoEditavel("EMPRESA", ""));
            html.AppendLine(LinhaInfoEditavel("CLIENTE", ""));
            html.AppendLine(LinhaInfoEditavel("MÁQUINA", ""));
            ...
            html.AppendLine("<table class='tabela-info'>");
            html.AppendLine(LinhaInfoEditavel("ORDEN DE TRABAJO", "Nº"));
            html.AppendLine(LinhaInfoEditavel("FECHA", dataGeracao.Split(' ')[0]));
            html.AppendLine(LinhaInfoEditavel("PLAZO DE ENTREGA", ""));
            html.AppendLine(LinhaInfoEditavel("CANT. PIEZAS", "Nº"));
            html.AppendLine(LinhaInfoEditavel("OPERADOR", "NOMBRE:"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            html.AppendLine("<div class='coluna-esquerda-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>DIMENSIONES DE LA MATERIA PRIMA</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LONGITUD", "X", temBloco ? blocoL.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("ANCHO", "Y", temBloco ? blocoW.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine(LinhaDimEditavel("ALTURA/ESPESOR", "Z", temBloco ? blocoH.ToString("F1") : "NaN", "td-valor-dim-materia"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");

            html.AppendLine("<div class='coluna-direita-topo'>");
            html.AppendLine("<div class='titulo-tabela-dim'>DIMENSIONES DE LA PIEZA</div>");
            html.AppendLine("<table class='tabela-dim'>");
            html.AppendLine(LinhaDimEditavel("LONGITUD", "X", temPeca ? pecaL.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("ANCHO", "Y", temPeca ? pecaW.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine(LinhaDimEditavel("ALTURA/ESPESOR", "Z", temPeca ? pecaH.ToString("F1") : "NaN", "td-valor-dim-peca"));
            html.AppendLine("</table>");
            html.AppendLine("</div>");
            ...
            if (temBloco || temPeca)
            {
                ...
                if (temBloco)
                {
                    ...
                    html.AppendLine("<div class='vista-titulo'>Materia Prima - Isométrica</div>");
                    html.AppendLine(GerarDiagramaBloco(blocoL, blocoW, blocoH, "materiaprima"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Longitud: {0:F0}mm &nbsp;|&nbsp; Ancho: {1:F0}mm &nbsp;|&nbsp; Altura: {2:F0}mm</div>", blocoL, blocoW, blocoH));
                    html.AppendLine("</div>");
                }
                ...
                {
                    html.AppendLine("<div class='vista-setup vista-dim'>");
                    html.AppendLine("<div class='vista-titulo'>Pieza - Isométrica</div>");
                    html.AppendLine(GerarDiagramaBloco(pecaL, pecaW, pecaH, "peca"));
                    html.AppendLine(string.Format("<div class='dim-legenda'>Longitud: {0:F0}mm &nbsp;|&nbsp; Ancho: {1:F0}mm &nbsp;|&nbsp; Altura: {2:F0}mm</div>", pecaL, pecaW, pecaH));
                    html.AppendLine("</div>");
                }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Pieza en Fijación - Vista Isométrica</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Pieza en Fijación')\" style='cursor:pointer; width:100%; height:500px; object-fit:fill;' src='data:image/jpeg;base64,{0}' alt='Pieza montada en la mordaza'/>", imagemPrincipal));
                html.AppendLine("</div>");
            }
            ...
            if (!string.IsNullOrEmpty(imagemPrincipal))
            {
                ...
                html.AppendLine("<div class='vista-titulo'>Configuración (fijación)</div>");
                html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Vista Principal')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='Vista principal isométrica con fijación'/>", imagemPrincipal));
                html.AppendLine("</div>");

                if (!string.IsNullOrEmpty(imagemOrigemWCS))
                {
                    html.AppendLine("<div class='vista-setup vista-principal setup-instrucoes-imagem-horizontal'>");
                    html.AppendLine("<div class='vista-titulo'>Origen de Trabajo (WCS / G54) - Vista Superior</div>");
                    html.AppendLine(string.Format("<img onclick=\"abrirLightbox(this.src, 'Origem WCS')\" style='cursor:pointer' src='data:image/jpeg;base64,{0}' alt='Origen de trabajo WCS'/>", imagemOrigemWCS));
                    html.AppendLine("</div>");
                }
            ...
            html.AppendLine("<tr><th>T#</th><th>Nombre</th><th class='num'>Diámetro</th><th class='num'>Longitud</th></tr>");
            ...
            // reduz a altura total pela metade quando ha muitas operaciones.
            int totalOps = operacionesOrdenadas.Count;
            int metade = (totalOps + 1) / 2;
            List<OperacaoOrdenada> primeiraMetade = operacionesOrdenadas.GetRange(0, Math.Min(metade, totalOps));
            List<OperacaoOrdenada> segundaMetade = totalOps > metade
                ? operacionesOrdenadas.GetRange(metade, totalOps - metade)
            ...
            html.AppendLine(string.Format(
                "<div class='resumo'>TOTAL GENERAL &nbsp;&nbsp;|&nbsp;&nbsp; Tiempo de corte: {0} &nbsp;&nbsp;|&nbsp;&nbsp; Tiempo total: {1}</div>",
                FormatarTempo(tempoCorteTotalGeral), FormatarTempo(tempoTotalGeral)));

            html.AppendLine("</div>"); // fim pagina-impressao 3

            // ================================================================
            // PAGINA 4: HERRAMIENTA DETALLADA
            // ================================================================
            html.AppendLine("<div class='pagina-impressao'>");
            html.AppendLine("<h2>HERRAMIENTA DETALLADA</h2>");
            html.AppendLine("<div class='vistas-container'>");

            foreach (KeyValuePair<string, DadosFerramenta> kv in ferramentas)
            {
                DadosFerramenta f = kv.Value;
                string operacionesLista = string.Join(", ", f.Operacoes.ToArray());
                ...
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd' colspan='2'>Operacao: {0}</td></tr>", operacionesLista));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Número de Herramienta:</td><td class='td-valor-fd'>{0}</td></tr>", f.NumeroFerramenta >= 0 ? f.NumeroFerramenta.ToString() : "-"));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Nombre de Herramienta:</td><td class='td-valor-fd'>{0}</td></tr>", f.Nome));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Diámetro de Herramienta:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.Diametro)));
                html.AppendLine(string.Format("<tr><td class='td-rotulo-fd'>Longitud Libre de Herramienta:</td><td class='td-valor-fd'>{0}</td></tr>", FormatarMm(f.ComprimentoCorte)));
                ...
                if (temBloco && f.ComprimentoCorte > 0)
                {
                    ...
                    string textoAlerta = alcancaProfundidade
                        ? string.Format("OK - alcanza {0:F0}mm de profundidad del bloque ({1:F0}mm)", blocoH, f.ComprimentoCorte)
                        : string.Format("ATENCIÓN - profundidad del bloque ({0:F0}mm) mayor que el alcance de la herramienta ({1:F0}mm)", blocoH, f.ComprimentoCorte);

                    html.AppendLine(string.Format(
                        "<tr><td class='td-rotulo-fd'>Voladizo Mínimo (estimado):</td><td class='td-valor-fd' style='color:{0};'>{1}</td></tr>",
            ...
            // quando ha muitas operaciones.
            // ================================================================
            List<OperacaoOrdenada> operacionesComImagemIPW = new List<OperacaoOrdenada>();
            foreach (OperacaoOrdenada op in operacionesOrdenadas)
            {
                if (!string.IsNullOrEmpty(op.ImagemIPW)) operacionesComImagemIPW.Add(op);
            }

            const int imagensPorPagina = 4;
            int totalPaginasImagens = (int)Math.Ceiling(operacionesComImagemIPW.Count / (double)imagensPorPagina);
            ...
            for (int p = 0; p < totalPaginasImagens; p++)
            {
                ...
                int fim = Math.Min(inicio + imagensPorPagina, operacionesComImagemIPW.Count);

                for (int i = inicio; i < fim; i++)
                {
                    OperacaoOrdenada op = operacionesComImagemIPW[i];
        ...
        private static string GerarTabelaOperacoes(List<OperacaoOrdenada> lista)
        {
            ...
            sb.AppendLine("<tr><th>Img</th><th>#</th><th>Operación</th><th>Herramienta</th><th class='num'>Tiempo de Corte</th><th class='num'>Tiempo Total</th></tr>");
